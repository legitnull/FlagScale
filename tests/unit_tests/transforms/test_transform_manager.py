...
sys.path.append("/share/project/fengyupu/github/FlagScale")
...
class TransformManagerTests(unittest.TestCase):
    ...
    def test_phase(self):
        manager = TransformManager(
            [self.transforms1, self.transforms3, self.transforms4], dry_run=True
        )
        expected_result = "DryRun plan(\nPre-compile transforms:\n  TestTransform1\nCompile transforms:\n  TestTransform3\nPost-compile transforms:\n  TestTransform4\n)"
        self.assertEqual(manager.apply(self.model1), expected_result)

    def test_supports(self):
        manager = TransformManager([self.transforms2], dry_run=True)
        with self.assertRaises(ValueError) as cm:
            manager.apply(self.model2)
        self.assertIn("Transform TestTransform2 not supported for this model.", str(cm.exception))

    def test_priority(self):
        manager = TransformManager(
            [self.transforms1, self.transforms2, self.transforms6, self.transforms7], dry_run=True
        )
        expected_result = "DryRun plan(\nPre-compile transforms:\n  TestTransform1\n  TestTransform2\nCompile transforms:\nPost-compile transforms:\n  TestTransform7\n  TestTransform6\n)"
        self.assertEqual(manager.apply(self.model1), expected_result)

    def test_preflight(self):
        manager = TransformManager([self.transforms9], dry_run=True)
        with self.assertRaises(ValueError) as cm:
            manager.apply(self.model1)
        self.assertIn("Transform TestTransform9 not supported.", str(cm.exception))

    def test_before_after(self):
        manager = TransformManager(
            [self.transforms4, self.transforms5, self.transforms6, self.transforms7], dry_run=True
        )
        expected_result = "DryRun plan(\nPre-compile transforms:\nCompile transforms:\nPost-compile transforms:\n  TestTransform7\n  TestTransform4\n  TestTransform6\n  TestTransform5\n)"
        self.assertEqual(manager.apply(self.model2), expected_result)

    def test_forbids(self):
        manager = TransformManager(
            [self.transforms1, self.transforms4, self.transforms2], dry_run=True
        )
        with self.assertRaises(ValueError) as cm:
            manager.apply(self.model1)
        self.assertIn(
    ...
    def test_requires(self):
        manager = TransformManager([self.transforms2, self.transforms3], dry_run=True)
        with self.assertRaises(ValueError) as cm:
            manager.apply(self.model1)
        self.assertIn(
    ...
    def test_wrong_phase(self):
        manager = TransformManager([self.transforms8], dry_run=True)
        with self.assertRaises(ValueError) as cm:
            manager.apply(self.model1)
