...
```sh
modelscope download --model lerobot/pi0_base --local_dir /share/pi0_base
...
```sh
# Download the dataset (already done in the Download Statistics section above)
# The dataset is located at: /share/lerobot/aloha_mobile_cabinet
```
...
- `model.checkpoint_dir` -> Path to pretrained model (e.g., `/share/pi0_base`)
- `model.tokenizer_path` -> Path to tokenizer (e.g., `/share/paligemma-3b-pt-224`)
- `model.tokenizer_max_length` -> Maximum tokenizer sequence length
- `model.action_steps` -> Number of action steps to predict

**Data settings**:
- `data.data_path` -> Path to LeRobot dataset root (e.g., `/share/lerobot/aloha_mobile_cabinet`)
...
**Engine settings:**
- `engine.model` -> Path to pretrained model (e.g., `/share/pi0_base`)
- `engine.tokenizer` -> Path to tokenizer (e.g., `/share/paligemma-3b-pt-224`)
- `engine.stat_path` -> Path to dataset statistics (e.g., `/share/lerobot/aloha_mobile_cabinet/meta/stats.json`)
