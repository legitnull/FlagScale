...
def extract_sample(
    ...
    if frame_index is not None:
        ...
        raise ValueError(
            "Must provide either frame_index or (episode_index, frame_in_episode)"
        )

    if idx >= len(dataset):
        raise ValueError(
            f"Index {idx} out of range (dataset has {len(dataset)} frames)"
        )
...
def get_args():
    parser = argparse.ArgumentParser(
        description="Extract inference inputs from LeRobotDataset"
    )
    parser.add_argument(
        "--dataset_root",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Output directory to save extracted files",
    )
    parser.add_argument(
        "--frame_index",
        type=int,
        default=None,
        help="Global frame index to extract",
    )
    parser.add_argument(
        "--episode_index",
        type=int,
...
def main():
    ...
    for idx, sample_name in zip(indices, sample_names, strict=False):
        ...
        paths = dump_sample(
            sample, output_dir, sample_name, args.image_format, dataset=dataset
        )
