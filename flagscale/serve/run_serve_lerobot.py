...
from flagscale.models.utils.constants import OBS_STATE, ACTION
from flagscale.runner.utils import logger
from flagscale.train.train_lerobot import make_pre_post_processors
from flagscale.models.configs.types import FeatureType, PolicyFeature, NormalizationMode
...
class PI0Server:
    ...
    def warmup(self):
        ...
        image_keys = self.config_generate.get(
            "images_keys",
            [
        ...
        images_shape = self.config_generate.get("images_shape", [3, 480, 640])
        for k in image_keys:
            batch[k] = torch.randn(batch_size, *images_shape, dtype=torch.float32).to(
                self.config_engine.device
            )
        state_key = self.config_generate.get("state_key", OBS_STATE)
        action_dim = self.config_generate.get("action_dim", 14)
    ...
    def load_model(self):
        ...
        for key, sub_dict in stats_dict.items():
            dataset_stats[key] = {
                k: torch.tensor(v).to(self.config_engine.device)
        ...
        if ACTION in dataset_stats:
            ...
            policy_config.output_features[ACTION] = PolicyFeature(
                ...
                shape=(actual_action_dim,),
            )
            logger.info(
                f"Set output_features[ACTION] to actual dimension: {actual_action_dim}"
            )

        # Get rename_map from config
        rename_map = self.config_generate.get("rename_map")
    ...
    def infer(self, batch):
        ...
        batch = {
            k: v.to(self.config_engine.device, non_blocking=True)
...
def infer_api():
    ...
    image_keys = PI0_SERVER.config_generate.get("images_keys", [])
    if not image_keys:
    ...
    state_key = PI0_SERVER.config_generate.get("state_key", OBS_STATE)
    if state.dim() == 1:
