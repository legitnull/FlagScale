...
def dump_runtime(
    ...
    if optimizer:
        ...
        for i, g in enumerate(optimizer.param_groups):
            print(
                f"group {i}: lr={g['lr']}, betas={g.get('betas')}, eps={g.get('eps')}, wd={g.get('weight_decay')}"
            )
    ...
    for _ in range(step, config.train_steps):
        ...
        if config.save_checkpoint and step % config.save_freq == 0:
            if is_main_process:
                ...
                checkpoint_dir = get_step_checkpoint_dir(output_dir, config.train_steps, step)
                policy_to_save = accelerator.unwrap_model(policy) if use_accelerator else policy.module
                save_checkpoint(
