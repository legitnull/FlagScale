...
def _warn_opaque_autograd_functions(targets: list[tuple[str, nn.Module]]) -> None:
    ...
    if opaque_fqns:
        logger.warning(
            f"Per-op SAC is applied to {len(opaque_fqns)} module(s) containing "
            "custom autograd functions (e.g. flash-linear-attention, Mamba). "
            "These ops are opaque to the ATen-level policy and will be fully "
            "recomputed during backward. Consider using mode='full' or layer-wise "
            f"selective AC for these layers. Affected: {opaque_fqns[:5]}"
        )
...
def apply_activation_checkpointing(
    ...
    *,
    ...
    if ac_config.mode == "none":
        logger.info("Activation checkpointing is disabled")
        return
    ...
    if ac_config.mode == "memory_budget":
        ...
        logger.info(f"Set activation memory budget to {ac_config.memory_budget}")
        return
    ...
    if not targets:
        logger.warning(
            "Activation checkpointing enabled but no modules matched. "
            "Check checkpoint_patterns or fsdp_units()."
        )
        return
    ...
    logger.info(
        f"Applied {ac_config.mode} activation checkpointing to "
        f"{wrapped_count}/{len(targets)} modules"
    )
